...
export const SectionEleven: React.FC<SectionElevenProps> = ({ scrollerRef }) => {
  ...
  const globeImgRef = useRef<HTMLImageElement>(null); // Globe image ref for VRAM management
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const setGlobeWillChange = () => { if (globeImgRef.current) globeImgRef.current.style.willChange = 'transform'; };
      const removeGlobeWillChange = () => { if (globeImgRef.current) globeImgRef.current.style.willChange = 'auto'; };
      ...
      mm.add("(min-width: 768px)", () => {
        ...
        // Scroll Scrub (Zoom + Rotate) with VRAM management
        gsap.fromTo(globeWrapperRef.current,
          {
            scale: 0.5,
            rotation: -30,
          },
          {
            ...
            scrollTrigger: {
              ...
              scrub: 1,
              onEnter: setGlobeWillChange,
              onEnterBack: setGlobeWillChange,
              onLeave: removeGlobeWillChange,
              onLeaveBack: removeGlobeWillChange
            }
          }
        );
      });

      // MOBILE: Simplified Entrance with Scroll Zoom (VRAM optimized)
