  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      ScrollTrigger.create({
        ...
        // Also trigger when scrolling back up for mobile
        onEnterBack: () => {
          if (!hasStartedTyping.current) {
            hasStartedTyping.current = true;
            let i = 0;
    ...
    <TechCard className="h-full min-h-[200px]">
      <div className="flex flex-col items-center text-center gap-4 md:gap-6 p-6 md:p-8 h-full">
        {/* 
            ...
            */}
        <div className="w-16 h-16 rounded-2xl bg-[#FC2C54]/10 border border-[#FC2C54] flex items-center justify-center shadow-none md:shadow-[0_0_20px_rgba(252,44,84,0.4)] md:group-hover:shadow-[0_0_40px_rgba(252,44,84,0.6)] transition-all duration-300 relative overflow-hidden shrink-0">
          {/* Scanline effect */}
          <div className="absolute inset-0 bg-[linear-gradient(to_bottom,transparent_50%,rgba(252,44,84,0.1)_50%)] bg-[length:100%_4px] pointer-events-none"></div>

          {/* 
                    Icon 
                    Mobile Optimization: Removed drop-shadow on mobile
                */}
          <TriangleAlert className="text-[#FC2C54] w-8 h-8 relative z-10 drop-shadow-none md:drop-shadow-[0_0_8px_rgba(252,44,84,1)]" strokeWidth={2} />
        </div>
