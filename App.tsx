        ...
        mm.add("(max-width: 767px)", () => {
            ...
            const tl = gsap.timeline({
                scrollTrigger: {
                    trigger: wrapperRef.current,
                    start: "top top",
                    end: "+=10%", // INSTANT: Minimal scroll completes transition
                    scrub: 0.05, // Near-instant response
                    pin: true,
                    // JIT will-change management for GPU memory optimization
                    onEnter: () => {
                        if (sec1Ref.current) sec1Ref.current.style.willChange = 'transform, opacity';
                        if (sec2Ref.current) sec2Ref.current.style.willChange = 'transform, opacity';
                    },
                    onLeave: () => {
                        setIsSectionTwoActive(true);
                        // FIXED: Keep will-change active to prevent mobile flicker/repaint
                    },
                    onEnterBack: () => {
            ...
            // Simplified Transition (Opacity & Scale only) with force3D
            tl
                .to(sec1Ref.current, {
                    scale: 1.1, // Reduced scale to prevent pixelation/layout shift
                    autoAlpha: 0,
                    force3D: true, // GPU acceleration priority
                    // No filter blur
                    duration: 1,
                    ease: "linear"
                }, 0)
